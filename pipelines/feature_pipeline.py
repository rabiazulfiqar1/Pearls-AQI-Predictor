...
Flow:
    1. Pull the last ~24 hours of fresh data from Open-Meteo (covers
...
def fetch_recent_air_quality(client) -> pd.DataFrame:
    ...
    Includes us_aqi alongside the raw pollutants — required because
    utils.features.engineer_features() now uses Open-Meteo's own us_aqi
    directly as the `aqi` column (rather than recomputing it), so it
    must be present in every fetch, not just the historical backfill.
    """
    params = {
...
def fetch_recent_weather(client) -> pd.DataFrame:
    ...
    Column names and the set of variables match utils.features.WEATHER
    exactly (temperature_2m, relative_humidity_2m, wind_speed_10m,
    pressure_msl, precipitation, cloud_cover) — the historical backfill
    fetches the same six, so both paths produce identically-shaped rows
    for the same Hopsworks feature group.
    """
    params = {
        "latitude": KARACHI_LAT,
        "longitude": KARACHI_LON,
...
def run_feature_pipeline() -> int:
    ...
    # the rest will be recomputed. Includes us_aqi now (see
    # fetch_recent_air_quality docstring).
    raw_cols = [
        "timestamp", "us_aqi", "pm2_5", "pm10", "no2", "o3", "so2", "co",
